function [zhat L ztilde Utilde,sensors, data] = F_sens_sel_KF_CRapproxnt(A, U, Q, sigma2, k, MAXITER,l)
% Solves the problem
...
for iter=1:MAXITER
    ...
    ind=[ind(end-lh+1:end) ; ind(randperm(m-lh))]; 
    ind=ind(1:l);   
    %tic
    %[P,~,~]=idare(A',U'*diag(sqrt(z)),Q,R,S,I)
    [~,Ssvd,Vsvd]=svd(diag(sqrt(z))*U,'econ');
    ...
    for i=1:l
        P1(:,:,i)=-sigma2*reshape(IAWinvPinv*vec(A*Winv*U(ind(i),:)'*U(ind(i),:)*Winv*A'),[n,n]);
        g(i)     =trace(P1(:,:,i)) - kappa*(1/z(ind(i)) - 1/(1-z(ind(i))));
    end
    %toc
    %tic
    for j=1:l
        for i=j:l
            L1 =  A*Winv*(-Pinv*P1(:,:,j)*Pinv+sigma2*U(ind(j),:)'*U(ind(j),:))*Winv*(-Pinv*P1(:,:,i)*Pinv+sigma2*U(ind(i),:)'*U(ind(i),:))*Winv*A';
            L2 =  A*Winv*(-Pinv*P1(:,:,i)*Pinv+sigma2*U(ind(i),:)'*U(ind(i),:))*Winv*(-Pinv*P1(:,:,j)*Pinv+sigma2*U(ind(j),:)'*U(ind(j),:))*Winv*A';
    ...
    H = H + kappa*diag(1./(z(ind).^2) + 1./((1-z(ind)).^2)) ; 
    %toc
    %tic
    ...
    Hinvg = (RR\(RR'\g));
    Hinv1 = (RR\(RR'\ones_l));
    dz = -Hinvg + ((ones_l'*Hinvg) / (ones_l'*Hinv1))*Hinv1;
    %toc
    %tic
    deczi = find(dz < 0);
    inczi = find(dz > 0);
    s = min([1; 0.99*[-z(ind(deczi))./dz(deczi) ; (1-z(ind(inczi)))./dz(inczi)]]);
